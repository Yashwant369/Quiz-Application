...
import com.yashwant.quiz_application.util.Question_Response;
...
//		}
		...
		for(QuizResponse res  : responses)
		{
			String qId = res.getQuestionId();
			String ans = res.getSubmitAnswer();
			map.put(qId, ans);
		}
		for(Question q : questions)
		{
			Question_Response res = new Question_Response();
			res.setQuestionTitle(q.getQuestionTitle());
			res.setQuestionId(q.getQuestionId());
			res.setCorrectAnswer(q.getRightAnswer());
			res.setOption1(q.getOption1());
			res.setOption2(q.getOption2());
			res.setOption3(q.getOption3());
			res.setOption4(q.getOption4());
			String qesId = q.getQuestionId();
			String submitAns = null;
			if(map.containsKey(qesId))
			{
				submitAns = map.get(qesId);
			}
			res.setSubmittedAnswer(submitAns);
			list.add(res);
		}
		
		reportResponse.setResponses(list);
