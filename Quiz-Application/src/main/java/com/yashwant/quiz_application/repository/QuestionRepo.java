...
public interface QuestionRepo extends JpaRepository<Question, String>{
	...
	@Query(value = "SELECT * FROM question WHERE category = :questionCategory AND difficult_level = :level ORDER BY RAND() LIMIT :medium", nativeQuery = true)
	List<Question> getMediumQuestion(Integer medium, String questionCategory, String level);
	@Query(value = "SELECT * FROM question WHERE category = :questionCategory AND difficult_level = :level ORDER BY RAND() LIMIT :hard", nativeQuery = true)
	List<Question> getHardQuestion(Integer hard, String questionCategory, String level);
